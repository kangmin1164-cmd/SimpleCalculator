...
![과제4 실행화면](img/screenshot-1.12.png)

![과제4 실행화면](img/screenshot-1.13.png)
...
5.  사용자 편의성: 사용자가 2(9+3)이라고만 쳐도 프로그램이 알아서 2*(9+3)으로 계산합니다. (암시적 곱셈)
6.  데이터 무결성: 화면에는 사용자가 입력한 2(9+3)을 그대로 보여주어 혼란을 방지하고, 내부적으로만 계산용 수식으로 변환합니다.
7.  안정성: Regex(정규 표현식)를 도입하여 복잡한 수식에서도 오류 없이 정확하게 곱셈 기호를 삽입하도록 설계했습니다.
